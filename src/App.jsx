...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 font-sans selection:bg-emerald-500/30">
      ...
      <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-[#020617]/80 backdrop-blur-md py-4 border-b border-emerald-500/10' : 'bg-transparent py-6'}`}>
          ...
          <div className="hidden md:flex gap-8 text-sm font-medium uppercase tracking-widest">
            {['home', 'skills', 'projects', 'contact'].map((item) => (
              <a 
                key={item}
                href={`#${item}`}
                className={`hover:text-emerald-400 text-decoration-none transition-colors ${activeSection === item ? 'text-emerald-500' : 'text-slate-400'}`}
          ...
          <div className="flex flex-col lg:flex-row items-center gap-16">
            <div className="flex-1 order-2 lg:order-1 text-center lg:text-left">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-bold mb-6 animate-fade-in">
                <item size={14} className="lucide lucide-sparkles" />
              ...
              <p className="text-lg md:text-xl text-slate-400 leading-relaxed mb-10 max-w-xl mx-auto lg:mx-0">
                ...
                {/* From architecting secure APIs to crafting responsive user interfaces, I turn complex technical requirements into elegant, functional reality. I thrive on solving difficult problems and staying at the forefront of emerging technology to deliver products that are built to scale. */}
              </p>

              <div className="flex flex-wrap justify-center lg:justify-start gap-4">
                <a href="#projects" className="text-decoration-none px-8 py-4 bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-bold rounded-xl transition-all hover:scale-105 flex items-center gap-2 shadow-lg shadow-emerald-500/20">
                  View My Work <ChevronRight size={20} />
                </a>
                <div className="flex gap-3">
                  {[Github, Linkedin, Mail].map((Icon, i) => (
                    <button key={i} className="p-4 bg-slate-800/50 hover:bg-slate-700 border border-slate-700 rounded-xl text-slate-300 transition-all hover:-translate-y-1">
                      <Icon size={24} />
                    </button>
                  ))}
                </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {skills.map((skill, i) => (
              ...
              >
                ...
                <span className="font-bold text-slate-200 tracking-wide text-sm">{skill.name}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            {projects.map((project, index) => (
              ...
              >
                ...
                <div className="p-6 relative">
                  ...
                  <button className="flex items-center gap-2 text-sm font-bold text-emerald-500 hover:text-emerald-400 transition-all group/btn">
                    View {project.link}
      ...
      <footer id="contact" className="bg-[#020617] border-t border-slate-900 pt-20 pb-10">
        <div className="max-w-7xl mx-auto px-6">
            ...
            <div className='lg:pl-10'>
              ...
              <ul className="space-y-4 list-none p-0">
                {['Home', 'Skills', 'Projects', 'Contact'].map(link => (
                  <li key={link}>
                    <a href={`#${link.toLowerCase().replace(' ', '')}`} className="no-underline text-slate-400 text-sm hover:text-emerald-500 transition-colors flex items-center gap-2 group">
                ...
                <li className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 text-sm m-0">
                      <a href={socialLinks.email} className="text-slate-400 hover:text-emerald-500 no-underline transition-colors tracking-tight">engineerogunbunmi@gmail.com</a>
                    </p>
                  </div>
                </li>
                <li className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 text-sm m-0">Available for hire</p>
                  </div>
                </li>
              </ul>
            </div>
          ...
          <div className="pt-8 border-t border-slate-900 flex flex-col md:flex-row justify-between items-center gap-4 text-slate-500 text-xs font-medium tracking-wider">
            <p>© {new Date().getFullYear()} JAYSMART PORTFOLIO.</p>
          </div>
        </div>
      </footer>
      <style>{`
